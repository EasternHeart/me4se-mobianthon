...
public class DateField extends Item {
	...
	public void setDate(Date date) {
		...
		if (date == null) {
			...
			Calendar c = Calendar.getInstance(timeZone);
			c.setTime(date);
			dateField.setText(digits(c.get(Calendar.YEAR), 4) + "-" + digits(c.get(Calendar.MONTH) + 1-Calendar.JANUARY, 2) + "-" + digits(c.get(Calendar.DAY_OF_MONTH) + 1, 2));

			timeField.setText(digits(c.get(Calendar.HOUR_OF_DAY), 2) + ":" + digits(c.get(Calendar.MINUTE), 2));
		}
	}
}
