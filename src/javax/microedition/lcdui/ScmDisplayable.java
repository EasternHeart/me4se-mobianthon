...
class ScmDisplayable extends ScmContainer {
	...
	static final Command MENU_COMMAND = new Command(ApplicationManager.manager.getProperty("menu.title.activate", "Menu"), Command.SCREEN, 0);

	/** 
	 * Note: Different from List.SELECT_COMMAND, this is used if a select
	 * command is required; *may* trigger a list select command  */
	
	static final Command ITEM_SELECT_COMMAND = new Command("Select", Command.OK , 0);
//	static final Command GAME_COMMAND1 = new Command("Action1", Command.SCREEN, 0);
//	static final Command GAME_COMMAND2 = new Command("Action2", Command.SCREEN, 0);
    ...
    public void init(){
        ...
        fullScreen = (displayable instanceof Canvas)
            && ((Canvas) displayable)._fullScreenMode;
...
//				displayable.handleCommand(bi.command, bi.item);
	...
	*/
	...
	protected void updateButtons() {
       ...
       // System.out.println("update buttons "+this.displayable+" current item: "+displayable.getCurrentItem());
        ...
        for (int i = 0; i < commands.size(); i++) {
            ...
            String desired[] = ApplicationManager
                    .split(ApplicationManager.manager
                            .getProperty("command.keys." + cmd.getTypeName()));
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            if (pass == 1) {
                remaining.insertElementAt(new CmdInfo(MENU_COMMAND, null), 0);
            }
            ...
            for (int i = 0; i < remaining.size(); i++) {
                ...
                String[] desired = ApplicationManager
                        .split(ApplicationManager.manager
                                .getProperty(cmd == MENU_COMMAND ? "command.menu.activate"
                                        : ("command.keys." + cmd.getTypeName())));

                if (cmd == MENU_COMMAND && desired.length == 0) {
            ...
            for (int i = 0; i < softButtons.length && remaining.size() > 0; i++) {
                if (softButtons[i].command == null) {
                    ...
                    if (softButtons[i].getWidth() >= ScmSoftButton.MIN_WIDTH) { // duplicate CK CMD
            ...
            if (remaining.size() == 0 || 
                   (remaining.size() == 1 && remaining.elementAt(0) == reinserted))
                break;
        }
