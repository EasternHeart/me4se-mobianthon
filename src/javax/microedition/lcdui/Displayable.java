...
//		noSelectButton = ApplicationManager.manager.properties.get("command.menu.activate")
	...
	void insertCommand(Vector v, Command cmd, Item item){
		...
		while(i < v.size() && ((CmdInfo) v.elementAt(i)).command.getPriority() < cmd.getPriority()) {
			i++;
		}
	 ...
	 * focused item, sorted by priority.
	 * 
	 * @ME4SE Internal */
	...
	Vector getCommandInfoList(){
		...
		if (selectButtonRequired 
			&& ApplicationManager.manager.getProperty(
					"command.menu.select", "").toUpperCase().startsWith("SOFT")) {
