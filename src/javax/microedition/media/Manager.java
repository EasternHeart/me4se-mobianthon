...
public final class Manager {
	...
	public static String[] getSupportedContentTypes(String s) {
		System.out.println("Manager.getSupportedContentTypes() called with no effect!");
		return new String[0];
	}
	...
	public static String[] getSupportedProtocols(String s) {
		System.out.println("Manager.getSupportedProtocols() called with no effect!");
		return new String[0];
	}
...
//        }
		...
		try {
			BasicMMPlayer player = (BasicMMPlayer) ApplicationManager.manager.instantiate(className);
			player.setSource((BasicMMDataSource) source);
			return player;
		}
		catch (ClassNotFoundException ex) {
			ex.printStackTrace();
			throw new MediaException("Cannot create a DataSource for: ContentType " + source.getContentType() + " not supported!. " + ex);
		}
	}
	...
	public static Player createPlayer(String locator) throws IOException, MediaException {
		System.out.println("Manager.createPlayer(" + locator + ")");
	...
	public static Player createPlayer(InputStream inputstream, String type) throws IOException, MediaException {
		System.out.println("Manager.createPlayer(" + inputstream + ", " + type + ")");
		BasicMMDataSource ds = new BasicMMDataSource(null);
		if (inputstream == null)
			throw new IllegalArgumentException("Cannot create Player for: InputStream must not be null !");
		if (type == null)
			throw new MediaException("Cannot create a Player for: cannot determine the media type");
	...
	public synchronized static void playTone(int note, int ms, int volume) throws MediaException {
		System.out.println("Manager.playTone(" + note + ", " + ms + ", " + volume +")");
		try {
			...
			ex.printStackTrace();
			throw new MediaException("No implementation found for Manager.playTone().");
		}
	}
